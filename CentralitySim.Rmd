...
# Function to compute edge-level statistics
calculate_edge_stats <- function(mat) {
  weights <- mat[mat > 0]
  if (length(weights) == 0) {
    return(data.frame(
      edge_count = 0,
  ...
  data.frame(
    edge_count = length(weights),
    total_weight = sum(weights),
    mean_weight = mean(weights),
    density = length(weights) / (nrow(mat) * ncol(mat)),
    cv_weight = sd(weights) / mean(weights),
    prop_strong = sum(weights > quantile(weights, 0.75)) / length(weights)
  )
}


# Loop through all simulations and all matrices
all_edge_stats <- lapply(seq_along(simulated_matrices), function(sim_id) {
  sim <- simulated_matrices[[sim_id]]
  
  lapply(names(sim), function(name) {
    stats <- calculate_edge_stats(sim[[name]])
    stats$Matrix <- name
    stats$SimID <- sim_id
    return(stats)
  }) %>% bind_rows()
...
summary_edge_stats_genus_level <- all_edge_stats %>%
  group_by(Matrix) %>%
  summarise(
    mean_edge_count = mean(edge_count, na.rm = TRUE),
    mean_total_weight = mean(total_weight, na.rm = TRUE),
    mean_mean_weight = mean(mean_weight, na.rm = TRUE),
    mean_density = mean(density, na.rm = TRUE),
    mean_cv_weight = mean(cv_weight, na.rm = TRUE),
    mean_prop_strong = mean(prop_strong, na.rm = TRUE),
    .groups = "drop"
  )
